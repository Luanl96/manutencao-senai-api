...
public class TecnicoService {
	...
	public Tecnico inserir(@Valid 
			@NotNull(message = "O técnico não pode ser nulo") Tecnico novoTecnico) {
		Preconditions.checkArgument(novoTecnico.isNovo(), "O técnico já foi salvo");
		Tecnico tecnicoSalvo = repository.save(novoTecnico);
		return tecnicoSalvo;
	}

	public Tecnico alterar(@Valid 
			@NotNull(message = "O técnico não pode ser nulo") Tecnico tecnicoSalvo) {
	...
	public List<Tecnico> listarPor(
			@NotEmpty(message = "O nome para busca não deve ser nulo") 
			@NotBlank(message = "O nome para busca não deve ser em branco") String nome) {
